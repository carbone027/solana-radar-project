...
import { formatJson } from '../helpers/FormatJsonHelper';
import CryptoJS from 'crypto-js'; // Importa a biblioteca para descriptografia
...
export class XMLProcessingResolver {
  ...
  async filterAndConvertXml(
    @Arg('features', () => [String]) encryptedFeatures: string[],
    @Arg('file', () => String) file: string
  ): Promise<TagValidationResult> {
    // Descriptografa as tags antes de processá-las
    const features = encryptedFeatures.map(tag =>
      CryptoJS.AES.decrypt(tag, XMLProcessingResolver.encryptionKey).toString(CryptoJS.enc.Utf8)
    );
